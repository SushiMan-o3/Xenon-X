...
def open_currency():
    with open('currency/currency.json') as f:
        balance = json.load(f)
        return balance
...
class currency(commands.Cog, name='Currency'):
    ...
    async def buy(self, ctx, *, item: str):
        with open('currency/currency.json') as f:
            balance = json.load(f)
        with open('currency/shop.json') as f:
            shop = json.load(f)
        item = item.lower()
    ...
    async def clear(self, ctx):
        balance = {}
        clearing = ['currency/items.json', 'currency/currency.json']
        for i in clearing:
            with open(i, 'w') as f:
                json.dump(balance, f, indent = 4)
        await ctx.send("All data is cleared.")
